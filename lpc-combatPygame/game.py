...
class Game:
    ...
    def play(
        self,
    ):  # TODO: Implements game loop (draw all elements (Screen, HUD, Tanks, Bricks))
        ...
        # Create screen
        screen = Screen(
            Dimension(Settings.SCREEN_WIDTH, Settings.SCREEN_HEIGHT))
        ...
        # Players
        tank_1 = Tank(
            Coordinate(Settings.SCREEN_WIDTH * (0.1), 320),
            Settings.SPRITES_PATH["PLAYER_1"],
            1,
        )
        tank_2 = Tank(
            Coordinate(Settings.SCREEN_WIDTH * (0.9), 320),
            Settings.SPRITES_PATH["PLAYER_2"],
            2,
        )
        ...
        brick_center_1 = Brick(
            Coordinate(Settings.SCREEN_WIDTH / 2 - 17.5, 180),
            Dimension(35, 80),
            Settings.COLORS["SCREEN_ORANGE"],
        )
        brick_center_2 = Brick(
            Coordinate(Settings.SCREEN_WIDTH / 2 - 17.5, 425),
            Dimension(35, 80),
            Settings.COLORS["SCREEN_ORANGE"],
        )
        brick_center_3 = Brick(
            Coordinate(Settings.SCREEN_WIDTH * (0.4) - 80, 325),
            Dimension(80, 30),
            Settings.COLORS["SCREEN_ORANGE"],
        )
        brick_center_4 = Brick(
            Coordinate(Settings.SCREEN_WIDTH * (0.6), 325),
            Dimension(80, 30),
            Settings.COLORS["SCREEN_ORANGE"],
        )

        brick_left_1 = Brick(
            Coordinate(120, 270), Dimension(
                17, 110), Settings.COLORS["SCREEN_ORANGE"]
        )
        brick_left_2 = Brick(
            Coordinate(103, 253), Dimension(
                34, 17), Settings.COLORS["SCREEN_ORANGE"]
        )
        brick_left_3 = Brick(
            Coordinate(103, 380), Dimension(
                34, 17), Settings.COLORS["SCREEN_ORANGE"]
        )

        brick_right_1 = Brick(
            Coordinate(680, 270), Dimension(
                17, 110), Settings.COLORS["SCREEN_ORANGE"]
        )
        brick_right_2 = Brick(
            Coordinate(680, 253), Dimension(
                34, 17), Settings.COLORS["SCREEN_ORANGE"]
        )
        brick_right_3 = Brick(
            Coordinate(680, 380), Dimension(
                34, 17), Settings.COLORS["SCREEN_ORANGE"]
        )
        ...
        while self.is_running:
            ...
            for ball in balls:
                if ball.hits == Settings.MAX_BALL_HITS:
                    balls.remove(ball)
                ...
                if (
                    ball.is_colliding(brick_left_1.coordinate,
                                      brick_left_1.dimension)
            ...
            bound(screen.surface, color=(Settings.COLORS["SCREEN_ORANGE"]))
            ...
            if keys[pygame.K_w]:
                if (
                    tank_1.is_colliding(
                        brick_left_1.coordinate, brick_left_1.dimension)
