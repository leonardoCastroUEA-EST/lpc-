...
class Nave:
    def __init__(self, photo, photo_damage, speed: int, x: float, y: float, ide: int):
        ...
        self.__vecto_max_fixed = self.__vecto_max
    ...
    def decelerator(self):
        if self.movement == 2:
            self.__vecto[0] -= self.__vecto[0] * 0.05
            self.__vecto[1] -= self.__vecto[1] * 0.05
    ...
    def spin_left(self) -> None:
        if self.__spin_left:
            ...
            self.__vecto_max = self.__vecto_max_fixed.rotate(self.__angle)
    ...
    def spin_by_axis(self, vector):
        if self.__spin_axis:
            vector_axis = pygame.Vector2(vector[0], vector[1])
            vector_init = pygame.Vector2(0, -2)
            self.__angle = vector_init.angle_to(vector_axis)
            self.__photo = pygame.transform.rotate(self.__photo_fixed, -self.__angle)
            self.__photo_damage = pygame.transform.rotate(self.__photo_damage_fixed, -self.__angle)
            self.__vecto_max = self.__vecto_max_fixed.rotate(self.__angle)
            self.__spin_axis = False
    ...
    def get_bullet_power(self):
        return self.__bullet_power

    def set_bullet_power(self, tf):
        self.__bullet_power = tf

    def set_time_bullet_power(self):
        self.__time_bullet_power = pygame.time.get_ticks()

    def get_time_bullet_power(self):
        return self.__time_bullet_power
