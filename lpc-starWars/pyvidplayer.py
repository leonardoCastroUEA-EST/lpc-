...
class Video:
    def __init__(self, patht: str):
        if not os.path.exists(patht):
            raise FileNotFoundError(ENOENT, os.strerror(ENOENT), patht)
        set_loglevel("quiet")
            
        self.path = patht
        self.name = os.path.splitext(os.path.basename(patht))[0]
        
        self._video = MediaPlayer(patht)
        self._frame_num = 0
        
        info = MediaInfo.parse(patht).video_tracks[0]
